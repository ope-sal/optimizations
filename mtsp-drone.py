...
mtsp_prob = LpProblem("MTSP-Model", LpMaximize)
...
#mtsp_prob += max([lpSum([cost(i,j)*use_edge[((i,j),k)] for i in range(n) for j in range(n) if i!=j]) for k in range(max_agents)])
